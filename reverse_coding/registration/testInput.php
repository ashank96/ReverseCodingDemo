...
function test_input(&$teamErr,&$lNameErr,&$lUsnErr,&$lEmailErr,&$lPhoneErr,&$mNameErr,&$mUsnErr,&$mEmailErr,&$mPhoneErr)
{
    ...
    else if(!preg_match("/^1[Ss][iI]1[0-7]([cCiI][sS]|[eE][eECciI]|[mMtT][eE]|[cC][hHvV]|[bB][tT]|[iI][mM])[0-9][0-9][0-9]$/",$_POST['leaderUsn'])){
        $lUsnErr.="*Invalid USN"; 
        $i=-1;
    }
    ...
    else if(!preg_match("/^1[Ss][iI]1[0-7]([cCiI][sS]|[eE][eECciI]|[mMtT][eE]|[cC][hHvV]|[bB][tT]|[iI][mM])[0-9][0-9][0-9]$/",$_POST['member1Usn'])){
        $mUsnErr.="*Invalid USN";
        $i=-1;
    }
